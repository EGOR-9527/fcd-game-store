...
const { TokenVendorModel } = require("../../model/model");
...
class TokenVendorService {
  ...
  async saveToken(userId, refreshToken) {
    const tokenData = await TokenVendorModel.findOne({ where: { userId } });

    if (tokenData) {
      tokenData.token = refreshToken;
      return tokenData.save();
    }
    const token = await TokenVendorModel.create({
      userId: userId,
      token: refreshToken,
    });
    return token;
  ...
  async removeToken(refreshToken) {
    const tokenData = await TokenVendorModel.destroy({
  ...
  async findToken(refreshToken) {
    const tokenData = await TokenVendorModel.findOne({
      where: { token: refreshToken },
    });
    return tokenData;
  }

  // Добавьте метод findOne
  async findOne(condition) {
    const tokenData = await TokenVendorModel.findOne(condition);
    return tokenData;
  }
}
